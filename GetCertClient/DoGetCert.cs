...
"
                    ...
                    if ( loProfile.bValue("-FetchSource", false) )
                        tvFetchResource.ToDisk(ResourceAssembly.GetName().Name, "GetCert.zip", null);
        ...
        private bool bRunPowerScript(out string asOutput, string asScript)
        {
            ...
            string lsProcessArgs = String.Format(moProfile.sValue("-PowerShellExeArgs", @"-NoProfile -ExecutionPolicy unrestricted -File ""{0}"""), lsScriptPathFile);
            string lsLogScript = !asScript.Contains("-CertificatePassword") ? asScript : asScript.Substring(0, asScript.IndexOf("-CertificatePassword")) + "\r\n";
        ...
        private static tvProfile oHandleUpdates(ref string[] args, ref tvMessageBox aoStartupWaitMsg)
        {
            ...
            try
            {
                        ...
                        && Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location).Contains(ResourceAssembly.GetName().Name) )
                {
                    ...
                    // Fetch WCF config.
                    tvFetchResource.ToDisk(ResourceAssembly.GetName().Name
                            , String.Format("{0}{1}", DoGetCert.sFetchPrefix, "GetCert.exe.config"), loProfile.sLoadedPathFile);
...
"
        ...
        public bool bGetCertificate()
        {
            ...
            try
            {
                ...
                lsCertPathFile = moProfile.sRelativeToProfilePathFile(moProfile.sValue("-CertificatePathFile", "cert.pfx"));
                ...
                if ( null == lbtArrayNewCertificate && !lbRepositoryCertsOnly )
                {
                    ...
                    else
                    {
                        ...
                        string  lsACMESharpFile = "ACMESharp.zip";
                        string  lsACMESharpPath = Path.GetFileNameWithoutExtension(lsACMESharpFile);
                        string  lsACMESharpPathFile = moProfile.sRelativeToProfilePathFile(lsACMESharpFile);

                        // Fetch ACMESharp module.
                        tvFetchResource.ToDisk(ResourceAssembly.GetName().Name
                                , String.Format("{0}{1}", DoGetCert.sFetchPrefix, lsACMESharpFile), lsACMESharpFile);
                        if ( !Directory.Exists(moProfile.sRelativeToProfilePathFile(lsACMESharpPath)) )
                            ZipFile.ExtractToDirectory(lsACMESharpPathFile, Path.GetDirectoryName(lsACMESharpPathFile));

                        // Don't use the powershell gallery installed ACMESharp module (by default, use what's embedded instead).
                        if ( !moProfile.bValue("-ACMESharpModuleUseGallery", false) )
                        {
                            string lsACMESharpPathFolder = moProfile.sValue("-ACMESharpPath", lsACMESharpPath);

                            moProfile.sValue("-ACMESharpPathHelp", String.Format(
                                    "Any alternative -ACMESharpPath must include a subfolder named \"{0}\" that contains the DLLs.", lsACMESharpPathFolder));

                            lsACMESharpPath = moProfile.sRelativeToProfilePathFile(moProfile.sValue("-ACMESharpPath", lsACMESharpPath));
                        }

                        if ( lbGetCertificate && !mbMainLoopStopped )
                        {
                            this.LogStage("1 - init vault");

                            string lsVaultPath = moProfile.sValue("-ACMESharpVaultPath", @"C:\ProgramData\ACMESharp\sysVault");
                            string lsVaultPathFile = Path.Combine(lsVaultPath, moProfile.sValue("-ACMESharpVaultFile", "00-VAULT"));

                            // Delete vault.
                            if ( Directory.Exists(lsVaultPath) )
                                Directory.Delete(lsVaultPath, true);

                            lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage1", @"
Import-Module ""{ACMESharpPath}""
Initialize-ACMEVault
                                    ")
                                    .Replace("{ACMESharpPath}", lsACMESharpPath)
                                    );

                            // Fix "vault" file (to use staging URL).
                            if ( lbGetCertificate && !mbMainLoopStopped && moProfile.bValue("-DoStagingTests", true) )
                                lbGetCertificate = this.bFixTextFile(lsVaultPathFile, File.ReadAllText(lsVaultPathFile)
                                        .Replace("\"BaseService\": \"LetsEncrypt\"", "\"BaseService\": \"LetsEncrypt-STAGING\"")
                                        .Replace(String.Format("\"BaseUri\": \"{0}\"", moProfile.sValue("-ServiceUrlLive", "https://acme-v01.api.letsencrypt.org/"))
                                                , String.Format("\"BaseUri\": \"{0}\"", moProfile.sValue("-ServiceUrlStaging", "https://acme-staging.api.letsencrypt.org/")))
                                        );
                        }

                        if ( lbGetCertificate && !mbMainLoopStopped )
                        {
                            this.LogStage("2 - register domain contact");

                            lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage2", @"
Import-Module ""{ACMESharpPath}""
New-ACMERegistration -Contacts mailto:{ContactEmailAddress} -AcceptTos
                                    ")
                                    .Replace("{ACMESharpPath}", lsACMESharpPath)
...
Import-Module ""{ACMESharpPath}""
New-ACMEIdentifier -Dns {CertificateDomainName} -Alias {CertificateDomainName}
Complete-ACMEChallenge {CertificateDomainName} -ChallengeType http-01 -Handler iis -HandlerParameters @{ WebSiteRef = '{DefaultSiteName}' }
                                        ")
                                        .Replace("{ACMESharpPath}", lsACMESharpPath)
                                        .Replace("{CertificateDomainName}", lsSanItem)
                                        .Replace("{DefaultSiteName}", loSanSite.Name)
                                        );

                                // Fix faulty IIS "web.config" prior to challenge submission.
                                if ( lbGetCertificate && !mbMainLoopStopped )
                                    lbGetCertificate = this.bFixTextFile(moProfile.sValue("-AcmeWebConfigPathFile", String.Format(@"C:\inetpub\wwwroot\{0}\web.config", lsAcmeBasePath))
    ...
    <system.webServer>
        <staticContent>
            <mimeMap fileExtension=""."" mimeType=""text/plain"" />
                            ...
                            if ( lbGetCertificate && !mbMainLoopStopped )
                            {
                                this.LogStage("4 - submit challenge to certificate provider");

                                lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage4", @"
Import-Module ""{ACMESharpPath}""
Submit-ACMEChallenge {CertificateDomainName} -ChallengeType http-01
                                        ")
                                        .Replace("{ACMESharpPath}", lsACMESharpPath)
                                        .Replace("{CertificateDomainName}", lsSanItem)
                                        );
                        ...
                        foreach(string lsSanItem in lsSanArray)
                        {
                            if ( lbGetCertificate && !mbMainLoopStopped )
                            {
                                for (int i=0; i < liSubmissionRetries; ++i)
                                {
                                    ...
                                    this.LogStage("5 - update challenge from certificate provider");

                                    string  lsSubmissionPending = ": pending";
                                    string  lsOutput = null;

                                    lbGetCertificate = this.bRunPowerScript(out lsOutput, moProfile.sValue("-ScriptStage5", @"
Import-Module ""{ACMESharpPath}""
(Update-ACMEIdentifier {CertificateDomainName} -ChallengeType http-01).Challenges | Where-Object {$_.Type -eq ""http-01""}
                                            ")
                                            .Replace("{ACMESharpPath}", lsACMESharpPath)
                                            .Replace("{CertificateDomainName}", lsSanItem)
                                            );
                        ...
                        if ( lbGetCertificate && !mbMainLoopStopped )
                        {
                            this.LogStage("6 - generate certificate request, submit certificate request");

                            // A SAN array of one item means there is the primary domain only.
                            if ( 1 == lsSanArray.Length )
                            {
                                lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage6", @"
Import-Module ""{ACMESharpPath}""
New-ACMECertificate {CertificateDomainName} -Generate -Alias cert1
Submit-ACMECertificate cert1
                            ...
                            {
                                string  lsSanList = ""; // Skip item 0 (primary domain is already represented).
                                        for (int i=1; i < lsSanArray.Length; i++)
                                            lsSanList += ("" == lsSanList ? "" : ",") + String.Format("'{0}'", lsSanArray[i]);

                                lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage6SanList", @"
Import-Module ""{ACMESharpPath}""
New-ACMECertificate {CertificateDomainName} -AlternativeIdentifierRefs ([array]$({CertificateSanList})) -Generate -Alias cert1
Submit-ACMECertificate cert1
                                        ")
                                        .Replace("{ACMESharpPath}", lsACMESharpPath)
                                        .Replace("{CertificateDomainName}", lsCertName)
                                        .Replace("{CertificateSanList}", lsSanList)
                                        );
                            }
                        }
                        ...
                        if ( lbGetCertificate && !mbMainLoopStopped )
                        {
                            for (int i=0; i < liSubmissionRetries; ++i)
                            {
                                ...
                                this.LogStage("7 - update certificate request");

                                string  lsSubmissionPending = "CertificateRequest       : \r\nCrtPemFile               :";
                                string  lsOutput = null;

                                lbGetCertificate = this.bRunPowerScript(out lsOutput, moProfile.sValue("-ScriptStage7", @"
Import-Module ""{ACMESharpPath}""
Update-ACMECertificate cert1
                        ...
                        if ( lbGetCertificate && !mbMainLoopStopped )
                        {
                            this.LogStage("8 - get certificate");

                            File.Delete(lsCertPathFile);

                            lbGetCertificate = this.bRunPowerScript(moProfile.sValue("-ScriptStage8", @"
Import-Module ""{ACMESharpPath}""
Get-ACMECertificate cert1 -ExportPkcs12 '{CertificatePathFile}' -CertificatePassword '{CertificatePassword}'
                                    ")
                                    .Replace("{ACMESharpPath}", lsACMESharpPath)
